...
class cisco_N77_C7706:
    def __init__(self,file):
        ...
        for line in read_file_list:
            #get ios version
            if re.findall('.*system image file is:\s+(bootflash:\S+)',line):
                iosversion = re.findall('.*system image file is:\s+(bootflash:\S+)',line)
        ...
        for line in read_file_list:
            ...
            if re.findall('.*System version:\s+(\S+)',line):
                version = re.findall('.*System version:\s+(\S+)',line)
                version = version[0]
                break
        ...
        for line in read_file_list:
            #CPU
            #cpu user
            if re.findall('^.*CPU states\s+:\s+(\d+.\d+)',line):
                cpu_break = True
                user = re.findall('^.*CPU states\s+:\s+(\d+.\d+)',line)
                user = float(user[0])
                #print('cpu')
                #print(cpu)
            #cpu kernel
            if re.findall('^.*CPU states\s+:\s+\d+.\d+%\s+user,\s+(\d+.\d+)',line):
                cpu_break = True
                kernel = re.findall('^.*CPU states\s+:\s+\d+.\d+%\s+user,\s+(\d+.\d+)',line)
        ...
        for i in list_cpu:
            try:
                ...
                sort_text =  re.findall('\d+\s+\d+\s+\d+\s+\d+\s+\d+.\d+%\s+\d+.\d+%\s+\d+.\d+%\s+-\s+(.*)',i)
        ...
        for i in read_file_list_env:
            if re.findall('^.*(Fan\d+\S+)\s+N77-\S+\s+\d+.\d+\s+\S+\s.*',i):
                regex_fan = re.findall('^.*(Fan\d+\S+)\s+N77-\S+\s+\d+.\d+\s+\S+\s.*',i)
                #tulis = input(i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('^.*Fan\d+\S+\s+N77-\S+\s+\d+.\d+\s+(.*)', i):
                regex_fan_cond = re.findall('^.*Fan\d+\S+\s+N77-\S+\s+\d+.\d+\s+(.*)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('^.*(\d+\s+Crossbar\S+)\s+\d+\s+\d+\s+\d+\s+.*',i):
                regex_temp = re.findall('^.*(\d+\s+Crossbar\S+)\s+\d+\s+\d+\s+\d+\s+.*',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^.*\d+\s+Crossbar\S+\s+\d+\s+\d+\s+\d+\s+(.*)', i):
                regex_temp_cond = re.findall('^.*\d+\s+Crossbar\S+\s+\d+\s+\d+\s+\d+\s+(.*)', i)

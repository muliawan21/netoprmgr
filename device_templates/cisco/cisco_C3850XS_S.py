...
class cisco_C3850XS_S:
    def __init__(self,file):
        ...
        for line in read_file_list:
            #get device model
            if re.findall('^cisco\s+(\S+).*processor.*with.*bytes',line):
        ...
        for line in read_file_list:
            #MEMORY
            #Memory Total
            if re.findall('^.*Processor Pool Total:\s+(\d+)\s+Used:\s+\d+\s+Free:\s+\d+',line):
                memory_break = True
                memory_total = re.findall('^.*Processor Pool Total:\s+(\d+)\s+Used:\s+\d+\s+Free:\s+\d+',line)
                memory_total = memory_total[0]
            #Memory Used
            if re.findall('^.*Processor Pool Total:\s+\d+\s+Used:\s+(\d+)\s+Free:\s+\d+',line):
                memory_used = re.findall('^.*Processor Pool Total:\s+\d+\s+Used:\s+(\d+)\s+Free:\s+\d+',line)

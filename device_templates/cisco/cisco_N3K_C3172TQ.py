...
class cisco_N3K_C3172TQ:
    def __init__(self,file):
        ...
        for line in read_file_list:
            #get device model
            if re.findall('PID:\s+(.*)-10GT',line):
                model = re.findall('PID:\s+(.*)-10GT',line)
                model = model[0]
                break                
        for line in read_file_list:
            #get ios version
            if re.findall('NXOS image file is:\s+(\S+.bin)',line):
                iosversion = re.findall('NXOS image file is:\s+(\S+.bin)',line)
        ...
        for line in read_file_list:
            ...
            if re.findall('System\s+version:\s+(\S+)',line):
                version = re.findall('System\s+version:\s+(\S+)',line)
                version = version[0]
                break
        ...
        for line in read_file_list:
            #CPU
            #cpu
            if re.findall('^CPU\s+util\s+:\s+(\d+)\S\d+%\s+user,\s+\d\S\d+%\s+kernel,\s+\d+\S\d+%\s+idle',line):
                cpu_break = True
                total = re.findall('^CPU\s+util\s+:\s+(\d+)\S\d+%\s+user,\s+\d\S\d+%\s+kernel,\s+\d+\S\d+%\s+idle',line)
                total = int(total[0])
                #print('cpu')
                #print(cpu)
            #cpu interrupt
            if re.findall('^CPU\s+util\s+:\s+\d\S\d+%\s+user,\s+(\d+)\S\d+%\s+kernel,\s+\d+\S\d+%\s+idle',line):
                interrupt = re.findall('^CPU\s+util\s+:\s+\d\S\d+%\s+user,\s+(\d+)\S\d+%\s+kernel,\s+\d+\S\d+%\s+idle',line)
        ...
        for line in read_file_list:
            ...
            if re.findall('.*PID\s+MemAlloc\s+MemLimit\s+MemUsed\s+StackBase\SPtr\s+Process',line):
        ...
        for i in read_file_list_env:
            if re.findall('(Fan\d\S+)\s+NXA-FAN-30CFM-F\s+\d+\S\d+\s+front-to-back\s.*',i):
                regex_fan = re.findall('(Fan\d\S+)\s+NXA-FAN-30CFM-F\s+\d+\S\d+\s+front-to-back\s.*',i)
                #tulis = input(i)
                fan = regex_fan[0]
                list_fan.append(fan)
                #print(fan)
            if re.findall('Fan\d\S+\s+NXA-FAN-30CFM-F\s+\d+\S\d+\s+front-to-back\s(.*)', i):
                regex_fan_cond = re.findall('Fan\d\S+\s+NXA-FAN-30CFM-F\s+\d+\S\d+\s+front-to-back\s(.*)', i)
                fan_cond = regex_fan_cond[0]
                list_fan_cond_cp.append(fan_cond)
                #print(fan_cond)
            if re.findall('^\d\s+(.*)\s+\d+\s+\d+\s+\d+\s+\S+',i):
                regex_temp = re.findall('^\d\s+(.*)\s+\d+\s+\d+\s+\d+\s+\S+',i)
                temp = regex_temp[0]
                list_temp.append(temp)
                #print(temp)
            if re.findall('^\d\s+.*\s+\d+\s+\d+\s+\d+\s+(\S+)', i):
                regex_temp_cond = re.findall('^\d\s+.*\s+\d+\s+\d+\s+\d+\s+(\S+)', i)
                temp_cond = regex_temp_cond[0]
                list_temp_cond.append(temp_cond)
                #print(temp_cond)
            if re.findall('^\d\s+(\S+)\s+AC\s+\d+\S\d+\s+\d+\S\d+\s+.*',i):
                regex_psu = re.findall('^\d\s+(\S+)\s+AC\s+\d+\S\d+\s+\d+\S\d+\s+.*',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(psu)
            if re.findall('^\d\s+\S+\s+AC\s+\d+\S\d+\s+\d+\S\d+\s+(.*)', i):
                regex_psu_cond = re.findall('^\d\s+\S+\s+AC\s+\d+\S\d+\s+\d+\S\d+\s+(.*)', i)

...
class cisco_ASR9010:
    def __init__(self,file):
        ...
        for line in read_file_list:
            ...
            if re.findall('^.*Cisco IOS XR Software,\s+Version\s+(\S+\d+)',line):
                version = re.findall('^.*Cisco IOS XR Software,\s+Version\s+(\S+\d+)',line)
                version = version[0]
                #print(version)
                break
        ...
        for line in read_file_list:
            #MEMORY
            #Memory Total
            if re.findall('.*Physical Memory:\s+(\d+)\s+MB',line):
                memory_break = True
                memory_total = re.findall('.*Physical Memory:\s+(\d+)\s+MB',line)
                memory_total = memory_total[0]
                list_memory_total.append(float(memory_total))
            #Memory Used
            if re.findall('.*Free Memory:\s+(\d+.\d+)\s+MB',line):
                memory_free = re.findall('.*Free Memory:\s+(\d+.\d+)\s+MB',line)
        ...
        for i in read_file_list_env:
            ...
            if re.findall('\d+\/PS\d+\/\w+\d+\/\*$',i):
                regex_psu = re.findall('\d+\/PS\d+\/\w+\d+\/\*$',i)
                psu = regex_psu[0]
                list_psu.append(psu)
                #print(psu)
            if re.findall('\s+host\s+PM\s+\d+\s+(\w+)', i):
                regex_psu_cond = re.findall('\s+host\s+PM\s+\d+\s+(\w+)', i)
